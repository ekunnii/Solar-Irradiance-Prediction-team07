...
class BasicBlock(tf.keras.layers.Layer):
    def __init__(self, filters, strides=(1, 1), **kwargs):
        ...
        if strides != (1, 1):
            self.shortcut = Conv2D(
                filters, (1, 1), name='projection', padding='same', use_bias=False)

        self.conv_0 = Conv2D(filters, (3, 3), name='conv_0',
                             strides=strides, padding='same', use_bias=False)
        self.conv_1 = Conv2D(filters, (3, 3), name='conv_1',
    ...
    def call(self, inputs, training):
        ...
        if self.strides != (1, 1):
            shortcut = tf.nn.avg_pool2d(net, ksize=(
...
class BottleneckBlock(tf.keras.layers.Layer):
    def __init__(self, filters, strides=(1, 1), projection=False, **kwargs):
        ...
        if projection or strides != (1, 1):
            self.shortcut = Conv2D(
                filters * 4, (1, 1), name='projection', padding='same', use_bias=False)

        self.conv_0 = Conv2D(filters, (1, 1), name='conv_0',
                             padding='same', use_bias=False)
        self.conv_1 = Conv2D(filters, (3, 3), name='conv_1',
                             strides=strides, padding='same', use_bias=False)
        self.conv_2 = Conv2D(filters * 4, (1, 1),
                             name='conv_2', padding='same', use_bias=False)
        self.bn_0 = BatchNormalization(name='bn_0', momentum=0.9, epsilon=1e-5)
        self.bn_1 = BatchNormalization(name='bn_1', momentum=0.9, epsilon=1e-5)
...
class ResNet_v2(tf.keras.models.Model):
    def __init__(self, layer_num, **kwargs):
        ...
        self.conv0 = Conv2D(64, (7, 7), strides=(
            2, 2), name='conv0', padding='same', use_bias=False)
        ...
        for layer_index, (b, f) in enumerate(zip(c.block_num[layer_num], c.filter_num), start=1):
            if layer_index == 1:
                if c.block_type[layer_num] == 'basic block':
                    ...
                    self.block_collector.append(self.block(
                        f, projection=True, name='conv1_0'))
            else:
                self.block_collector.append(self.block(
                    f, strides=(2, 2), name='conv{}_0'.format(layer_index)))

            for block_index in range(1, b):
                self.block_collector.append(self.block(
                    f, name='conv{}_{}'.format(layer_index, block_index)))

        self.bn = BatchNormalization(name='bn', momentum=0.9, epsilon=1e-5)
        self.global_average_pooling = GlobalAvgPool2D()
        self.fc = Dense(c.category_num, name='fully_connected',
    ...
    def call(self, inputs, training):
        ...
        net = tf.nn.max_pool2d(net, ksize=(
