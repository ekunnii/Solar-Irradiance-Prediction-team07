...
def generate_all_predictions(
    ...
    for station_idx, station_name in enumerate(target_stations):
        ...
        assert len(station_preds) == len(
            target_datetimes), "number of predictions mismatch with requested datetimes"
...
def main(
    ...
    if "bypass_predictions_path" in admin_config and admin_config["bypass_predictions_path"]:
        ...
        assert os.path.isfile(
            preds_output_path), f"invalid preds file path: {preds_output_path}"
        ...
        predictions = np.asarray([float(ghi)
                                  for p in predictions for ghi in p.split(",")])
    else:
        ...
        with open(preds_output_path, "w") as fd:
            for pred in predictions:
                fd.write(
                    ",".join([f"{v:0.03f}" for v in pred.tolist()]) + "\n")
    ...
    ), "user predictions should NOT contain NaN values"
    ...
    gt = gt.reshape((len(target_stations), len(
        target_datetimes), len(target_time_offsets)))
    day = parse_nighttime_flags(
        target_stations, target_datetimes, target_time_offsets, dataframe)
    day = day.reshape((len(target_stations), len(
        target_datetimes), len(target_time_offsets)))
